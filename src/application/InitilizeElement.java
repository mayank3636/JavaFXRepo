...
//			appError=new Label();
		...
		Thread t= new ProcessBuildDemo();		
		if(combobox.getValue()!=null&&testName.getText().isEmpty()==false) {
			
			RunTest.setVisible(false);
			stoptest.setVisible(true);
		stoptest.setOnAction(e2->{
			
			System.exit(0);
			t.stop();
		});
		ILTestName=testName.getText();
